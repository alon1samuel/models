...
tf.app.flags.DEFINE_bool(
    'evaluate_loop', False,
    'if true than evaluate with a loop on new checkpoint files.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.gpu_usage_fraction == 0:
        os.environ["CUDA_VISIBLE_DEVICES"] = ""
        config=None
    else:
        config = tf.ConfigProto()
        config.gpu_options.per_process_gpu_memory_fraction = FLAGS.gpu_usage_fraction

    if FLAGS.evaluate_loop:
        tf.logging.info('Evaluate in a loop.')
        while True:
            checkpoint_path = checkpoint_finder()
